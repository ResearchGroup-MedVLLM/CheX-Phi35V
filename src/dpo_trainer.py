...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[Union[list[int], Any, dict[str, Any]]]) -> dict[str, Any]:
        ...
        if "attention_mask" in examples[0]:
            pixel_attention_mask = [torch.tensor(example["attention_mask"]) for example in examples]
        ...
        if "attention_mask" in examples[0]:
            output["attention_mask"] = pad(pixel_attention_mask, padding_value=0)
...
class DPOTrainer(Trainer):
    ...
    def _prepare_dataset(
        ...
        # 缓存文件夹
        # num_crops=16的24k
        # cache_dir = "/workspace/01_Program/dpo_cache_24k_numc_16"
        # hard_2w
        # cache_dir = "/workspace/01_Program/dpo_cache_35k"
        # cache_dir = "/workspace/01_Program/random_20k"
        cache_dir = "/workspace/01_Program/random_1k"
        os.makedirs(cache_dir, exist_ok=True)
        # Build the kwargs for the `map` function
        map_kwargs = {"writer_batch_size": 256}
        ...
        with PartialState().local_main_process_first():
            ...
            dataset = dataset.map(
                ...
                # cache_file_name="cache_file.arrow",
                cache_file_name=os.path.join(cache_dir, f"{dataset_name}_processed.arrow"),
    ...
    def tokenize_row(features, processing_class, max_prompt_length, max_completion_length, add_special_tokens):
        ...
        bos_token_tensor = torch.tensor([[tokenizer.bos_token_id]])
        eos_token_tensor = torch.tensor([[tokenizer.eos_token_id]])

        # 添加特殊 token
        if add_special_tokens:
            if tokenizer.bos_token_id is not None:
                prompt_input_ids = torch.cat([bos_token_tensor, prompt_input_ids], dim=1)
            if tokenizer.eos_token_id is not None:
                prompt_input_ids = torch.cat([prompt_input_ids, eos_token_tensor], dim=1)

        chosen_input_ids = torch.cat([chosen_input_ids, eos_token_tensor], dim=1)
        rejected_input_ids = torch.cat([rejected_input_ids, eos_token_tensor], dim=1)
    ...
    def process_row(features, processing_class, max_prompt_length, max_completion_length, add_special_tokens):
        ...
        # print(features)
        processed_features = processor(images=features["images"], text=prompt, add_special_tokens=False, return_tensors="pt")
        
        # for key, tensor in processed_features.items():
        ...
        #     else:
        #         print(f"Key: {key} 的值不是 torch.Tensor")
        #     print("-" * 30)
        ...
        # print('Shape of prompt_input_ids:', prompt_input_ids.shape)


        chosen_input_ids = tokenizer(features["chosen"]+'<|end|>\n<|endoftext|>', add_special_tokens=False, return_tensors='pt')["input_ids"].squeeze(0)
        rejected_input_ids = tokenizer(features["rejected"]+'<|end|>\n<|endoftext|>', add_special_tokens=False,  return_tensors='pt')["input_ids"].squeeze(0)
        ...
        #     output["attention_mask"] = processed_features["attention_mask"][0]
    ...
    def concatenated_inputs(
        ...
        Args:
            batch (`dict[str, Union[list, torch.LongTensor]]`):
                ...
                - `"prompt_attention_mask"` (optional): Tensor for pixel attention masks, if available.
        ...
        Returns:
            `dict[str, torch.LongTensor]`: A dictionary containing:
                ...
                - `"attention_mask"` (optional): Concatenated pixel attention masks if `"prompt_attention_mask"` are present.
        ...
        if batch["prompt_input_ids"].ndim == 3:
            batch["prompt_input_ids"] = batch["prompt_input_ids"].squeeze(dim=0)

        if batch["prompt_attention_mask"].ndim == 3:
            batch["prompt_attention_mask"] = batch["prompt_attention_mask"].squeeze(dim=0)
        ...
        if "pixel_values" in batch:
            if batch["pixel_values"].ndim == 6:
                batch["pixel_values"] = batch["pixel_values"].squeeze(dim=0)
            output["pixel_values"] = torch.cat([batch["pixel_values"], batch["pixel_values"]], dim=0)

        if "attention_mask" in batch:
            if batch["attention_mask"].ndim == 3:
        ...
        if "image_sizes" in batch:
            if batch["image_sizes"].ndim == 3:
                batch["image_sizes"] = batch["image_sizes"].squeeze(dim=0)
            output["image_sizes"] = torch.cat([batch["image_sizes"], batch["image_sizes"]], dim=0)

        # print('This is in concatenated_inputs of dpo_trainer.py')
        # print('Shape of pixel_values:', output["pixel_values"].shape)
        # print('Shape of pixel_attention_mask:', output["attention_mask"].shape)
        # print('Shape of image_sizes:', output["image_sizes"].shape)
        # print('\n\n\n')
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor]]):
        ...
        """
        # print shape
        # print("Prompt input ids shape:", batch["prompt_input_ids"].shape)
        ...
        if "attention_mask" in concatenated_batch:
            model_kwargs["attention_mask"] = concatenated_batch["attention_mask"]
        ...
        # print('Shape of pixel_values:', concatenated_batch["pixel_values"].shape)
        # print('Shape of image_sizes:', concatenated_batch["image_sizes"].shape)
        # print('Shape of prompt_input_ids:', prompt_input_ids.shape)
        # print('Shape of prompt_attention_mask:', prompt_attention_mask.shape)
        # print('Shape of completion_input_ids:', completion_input_ids.shape)
        # print('Shape of completion_attention_mask:', completion_attention_mask.shape)
        # print('This is in dpo_trainer.py\n\n\n')
